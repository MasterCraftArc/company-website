...
const updateNav = (sectionRefs, refs) => {
  ...
  const selectedRef = sectionRefs.reduce(
    (previousRef, currentRef, index) => {
      return refs[index].current && window.scrollY > refs[index].current.offsetTop - 50
        ? currentRef
        : previousRef;
    },
    [sectionRefs[0]]
          ...
          {sectionNames.map((title, i) => {
            return (
              <li key={i + 1} className="">
                <a aria-hidden="true" ref={sectionRefs[i]} href={`#section${i + 1}`}>
                  {title}
                </a>
              </li>
            );
